"""Scrapes SEC filing detail pages for URLs leading to data.
"""
...
from scrape.workflows import process_filing_task
...
class Command(BaseCommand):
    ...
    help = "Scrapes SEC filing webpages."
    ...
    def handle(self, *args, **options) -> None:
        ...
        self._logger.info(
            "Received request to scrape SEC filing detail webpages."
        )

        # Fetch scraped filings not associated with completed task
        self._logger.info("Fetching unprocessed filings.")
        filings = ScrapedCompanyService.get_unprocessed_filings()
        self._logger.info(f"{len(filings):,} filing(s) found.")
        ...
        for batch in chunked(filings, n=EDGAR_RATE_LIMIT_IN_SECONDS):
        ...
        # Log success
        self._logger.info("Filing detail pages scrape completed successfully.")
